 ...
 *
 * Generates research questions based on analysis.
 * Minimal UI.
 */
...
interface QuestionGeneratorNodeProps {
  id: string;
  data: any;
  selected?: boolean;
}
...
export function QuestionGeneratorNode({ id, data, selected }: QuestionGeneratorNodeProps) {
  ...
  const handleGenerate = useCallback(async () => {
    ...
    try {
      ...
      const result = [
        'How does this methodology compare to previous approaches?',
        'What are the scalability limitations of the proposed system?',
        'Can the results be replicated with different datasets?',
        'What future research directions does this enable?',
        'How might bias in the training data affect outcomes?',
      ];
    ...
    <BaseNode id={id} data={data} selected={selected}>
      <div className="space-y-2">
        ...
        <div className="flex items-center justify-between">
          ...
          {questions.length > 0 && (
            <span className="text-gray-400 text-[10px]">{questions.length}</span>
          )}
        </div>
        ...
        {questions.length > 0 && (
          <div className="space-y-1 max-h-32 overflow-y-auto">
            {questions.map((q, i) => (
              <div key={i} className="p-1.5 bg-gray-50 rounded border border-gray-200 text-[10px]">
                <span className="text-gray-400 font-medium">{i + 1}.</span> {q}
              </div>
            ))}
          </div>
        )}
